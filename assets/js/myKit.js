        ...
        var myKit = {
            ...
            config: {
                ...
                template: {
                    ...
                    login: ['<div class="status-inner">', '    <div class="com-login"></div>', '    <span class="btn btn-close"></span>', '</div>'].join("")
                },
                carContent: []
            },
             ...
             * 显示/关闭加载
             * @param text 显示的文字
             * @param showMask 是否显示mask
             * @param autoHide 自动隐藏
             */
            showLoading: function (text, showMask, autoHide) {
                var loaders = document.querySelector('.vLoading');
                if (loaders) {
                    return false;
                } else {
                    var loaderDom = document.createElement('div');
                    loaderDom.className = 'vLoading';
                    showMask ? loaderDom.classList.add('msk') : null;
                    loaderDom.innerHTML = '<div class="t-wrp"><div class="t-core"><div class="t-loader">' +
                        '<div class="loader-inner line-spin-fade-loader"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div>' +
                        (text ? '<p>' + text + '</p>' : '') +
                        '</div></div></div>';
                    document.body.appendChild(loaderDom);
                    $('html,body').addClass('disable-scroll');
                    if (autoHide) {
                        setTimeout(function () {
                            var loaders = document.querySelector('.vLoading');
